...
- kpt packages live under `catalog` in this repository with a root `Kptfile` describing the stack.
...
All cluster-specific Porch manifests live under `modules/nixos/rke2lab/system/porch` so they can be applied together and versioned alongside the catalog.
...
spec:
  type: git
  git:
    repo: git@github.com:nxmatic/rke2lab.git
    branch: develop
    directory: modules/nixos/rke2lab/catalog
    secretRef:
      name: github-token
  deployment: false
----
...
spec:
  ...
  git:
    ...
    branch: main
    directory: packagevariants/bioskop
    createBranch: true
    secretRef:
...
|mesh/headscale |`cluster-name`, `lan-lb-pool`, `headscale-lb-ip`, `vip-network-cidr`, `darwin-host` |`HomeLanConfig`, `HeadscaleAuthConfig`
...
Config Sync (example) watches the deployment repo path containing published packages (for example `packagevariants/bioskop/**`). Each Porch-published `PackageRevision` becomes a directory commit that the agent applies.
...
.. Create Kubernetes Secrets (for example `github-token`) with deploy keys for catalog and deployment repos.
