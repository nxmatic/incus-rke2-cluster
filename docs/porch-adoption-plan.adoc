...
All cluster-specific Porch manifests live under `modules/nixos/incus-rke2-cluster/system/porch` so they can be applied together and versioned alongside the catalog.
...
spec:
  type: git
  git:
    repo: git@github.com:nxmatic/incus-rke2-cluster.git
    branch: develop
    directory: modules/nixos/incus-rke2-cluster/catalog
    secretRef:
      name: github-token
  deployment: false
----
