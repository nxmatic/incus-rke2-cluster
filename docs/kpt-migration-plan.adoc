...
1. **Headscale Server & Client** ✅ (from `cloud-config.master.headscale.yaml`)
   - Deployed via kpt package: `catalog/mesh/headscale/`
...
2. **Envoy Gateway** ✅ (from `cloud-config.master.cilium.yaml`)
   - Deployed via kpt package: `catalog/networking/envoy-gateway/`
...
=== Phase 2: Load Balancer (Medium Risk) ✅ COMPLETE

Migrate Kube-VIP after proving kpt workflow:

3. **Kube-VIP DaemonSet** ✅ (from `cloud-config.master.kube-vip.yaml`)
   - Deployed via kpt package: `catalog/ha/kube-vip/`
   - VIP 10.80.15.1 on vmnet0 interface stable
   - 6 resources tracked with ResourceGroup inventory
   - NodePort 30443 backup for emergency access
   - Fresh deployment verified with no RKE2 conflicts

**Success Criteria**: ✅ Achieved - VIP responding, control plane accessible, failover ready

=== Phase 3: Networking & Storage (High Risk)

Migrate core infrastructure components:

4. **Cilium Advanced Features** ✅ COMPLETE (from `cloud-config.master.cilium.yaml`)
   - ✅ Core CNI HelmChartConfig kept safely in RKE2 bootstrap
   - ✅ Deployed via kpt package: `catalog/networking/cilium/`
   - ✅ IP Pools: LoadBalancer (10.80.8.128/26, 64 IPs), VIP (10.80.15.0/24, 256 IPs)
   - ✅ L2 Announcements: ARP on vmnet0/lan0 interfaces
   - ✅ BGP: Peering ASN 65001 ↔ 65000 @ 10.80.0.1
   - ✅ LoadBalancer IPAM verified: test service got 10.80.8.130
   - ✅ 5 resources tracked with ResourceGroup inventory
   - ✅ Updated to stable v2 API versions (no deprecation warnings)

5. **OpenEBS ZFS** ⚠️ DEFERRED (from `cloud-config.master.base.yaml`)
   - Storage provisioner migration requires careful PV handling
   - **Decision**: Keep in RKE2 due to high risk (affects persistent volumes)
   - Alternative: Consider migration in future with proper PV migration tooling

**Success Criteria**: ✅ Achieved - Network connectivity maintained, LoadBalancer IPAM working, BGP config deployed

== Migration Status Summary

**Completion Date**: 2025-11-19

**Overall Status**: ✅ **PHASES 1-3a COMPLETE**

**Components Migrated to kpt**:

* ✅ Headscale (mesh networking) - 16 resources
* ✅ Envoy Gateway (Gateway API) - 5 resources  
* ✅ Kube-VIP (control plane HA) - 6 resources
* ✅ Cilium Advanced Features (BGP, L2, IP pools) - 5 resources

**Total**: 32 resources under declarative kpt management

**Components Remaining in RKE2** (by design):

* Core CNI (Cilium HelmChartConfig) - Required for cluster bootstrap
* OpenEBS ZFS - Deferred due to PV migration risk
* Traefik - Basic ingress for cluster initialization
* Tailscale - Cluster connectivity

**Key Achievements**:

* Clean separation between bootstrap (RKE2) and operational (kpt) components
* All high-risk migrations completed successfully
* No cluster downtime during migration
* Full rollback capability maintained
* LoadBalancer IPAM functional with Cilium IP pools
* BGP peering configured for service advertisement
...
pipeline:
  validators:
    - image: ghcr.io/kptdev/krm-functions-catalog/kubeval:v0.3
    - image: ghcr.io/kptdev/krm-functions-catalog/validate-rolebinding:v0.1
----
...
upstream:
  type: git
  git:
    repo: https://github.com/nxmatic/incus-rke2-cluster
    directory: modules/nixos/incus-rke2-cluster/catalog/mesh/headscale
    ref: develop
pipeline:
  mutators:
...
**To**: Multiple files in `catalog/mesh/headscale/`

**File 1**: `catalog/mesh/headscale/namespace.yaml`
...
**File 2**: `catalog/mesh/headscale/configmaps.yaml`
...
[source,bash]
----
kpt fn eval catalog/mesh/headscale \
  --image ghcr.io/kptdev/krm-functions-catalog/apply-setters:v0.2 \
  -- \
  home-lan-lb-ip=192.168.1.193 \
  headscale-version=v0.27.0 \
...
kpt-validate: kpt-render
	@kpt fn eval $(KPT_OUTPUT_DIR) --image ghcr.io/kptdev/krm-functions-catalog/kubeval:v0.3
...
make kpt-validate
kubectl apply --dry-run=server -f .local.d/rendered/
----
