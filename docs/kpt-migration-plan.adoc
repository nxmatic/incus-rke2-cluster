...
1. **Headscale Server & Client** ✅ (from `cloud-config.master.headscale.yaml`)
   - Deployed via kpt package: `kpt/catalog/mesh/headscale/`
...
2. **Envoy Gateway** ✅ (from `cloud-config.master.cilium.yaml`)
   - Deployed via kpt package: `kpt/catalog/networking/envoy-gateway/`
...
3. **Kube-VIP DaemonSet** ✅ (from `cloud-config.master.kube-vip.yaml`)
   - Deployed via kpt package: `kpt/catalog/ha/kube-vip/`
...
4. **Cilium Advanced Features** ✅ COMPLETE (from `cloud-config.master.cilium.yaml`)
   ...
   - ✅ Deployed via kpt package: `kpt/catalog/networking/cilium/`
...
upstream:
  ...
  git:
    ...
    directory: modules/nixos/incus-rke2-cluster/kpt/catalog/mesh/headscale
    ref: develop
pipeline:
  mutators:
...
**To**: Multiple files in `kpt/catalog/mesh/headscale/`

**File 1**: `kpt/catalog/mesh/headscale/namespace.yaml`
...
**File 2**: `kpt/catalog/mesh/headscale/configmaps.yaml`
...
[source,bash]
----
kpt fn eval kpt/catalog/mesh/headscale \
  --image ghcr.io/kptdev/krm-functions-catalog/apply-setters:v0.2 \
  -- \
  home-lan-lb-ip=192.168.1.193 \
  headscale-version=v0.27.0 \
