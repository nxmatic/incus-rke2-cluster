...
start
* *Change*: Manifest moved into `+kpt/system/traefik-config+` and is
...
* *Migration Strategy*:
[arabic]
. Create `+kpt/catalog/mesh/tailscale/+` package
...
`+kpt/system/cilium-config+` (with setters for cluster name/id)
...
|Traefik Config |master.base |✅ Done (kpt/system/traefik-config) |High
|CRITICAL |Applied by rke2-traefik-config.service
...
|Cilium CNI Config |master.cilium |✅ Done (kpt/system/cilium-config)
|High |CRITICAL |Applied by rke2-cilium-config.service
