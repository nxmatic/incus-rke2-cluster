...
start
* *Change*: Manifest moved into `+kpt/system/traefik+` and is
...
`+kpt/system/cilium+` (with setters for cluster name/id)
...
|Traefik Config |master.base |✅ Done (kpt/system/traefik) |High
✅ *Envoy Gateway* - Migrated to
|Cilium CNI Config |master.cilium |✅ Done (kpt/system/cilium)
