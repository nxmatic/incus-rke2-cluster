...
[source,yaml]
----
/etc/systemd/system/rke2-traefik-config.service ➜ ${CLUSTER_STATE_DIR}/system/traefik
----

*Assessment*: *Bootstrap-critical, now sourced from kpt*

* *Reason*: Cluster still needs ingress before Porch/Flux render loops
start
* *Change*: Manifest moved into `+system/traefik+` and is
applied by `+rke2-traefik-config.service+`
* *Benefit*: Values now live in git, can be reviewed/versioned like
other packages without inflating NoCloud user-data
* *Risk*: Remains HIGH if package missing—service has
`+ConditionPathExists+` to guard against empty state
* *Operational note*: `+rke2-kpt-package-sync+` mirrors the package
before the service runs, preserving the bootstrap order
...
* *Migration Strategy*:
[arabic]
. Create `+catalog/mesh/tailscale/+` package
. Extract namespace, HelmChart, and Connector resources
. Use kpt setters for: cluster-name, oauth-client-id,
oauth-client-secret, vip-cidr, lb-cidr
. Deploy on fresh master, verify Tailscale connectivity
. Remove from cloud-config.master.base.yaml
...
[source,yaml]
----
/etc/systemd/system/rke2-cilium-config.service ➜ ${CLUSTER_STATE_DIR}/system/cilium
----

*Assessment*: *Still core bootstrap, but now mirrored via kpt*

* *Reason*: CNI must reconcile before anything else; service order
enforces this
* *Change*: The HelmChartConfig left NoCloud and lives in
`+system/cilium+` (with setters for cluster name/id)
* *Benefit*: Declarative history + ability to reuse the same package
across clusters while passing setters via `+rke2-kpt-deploy+`
* *Risk*: CRITICAL if package absent; systemd unit will fail fast so
troubleshooting happens outside NoCloud space
* *Status*: Advanced Cilium features remain in dedicated packages; only
the bootstrap slice runs here

==== Previously Migrated (Now in kpt)

✅ *Headscale* - Migrated to `+catalog/mesh/headscale/+`

|Traefik Config |master.base |✅ Done (system/traefik) |High
✅ *Envoy Gateway* - Migrated to
|Cilium CNI Config |master.cilium |✅ Done (system/cilium)
`+catalog/networking/envoy-gateway/+`

✅ *Kube-VIP* - Migrated to `+catalog/ha/kube-vip/+`

✅ *Cilium Advanced Features* - Migrated to
`+catalog/networking/cilium/+`
...
* Connector resource (subnet router for control plane VIP + LoadBalancer
pool)
...
[arabic]
. *Traefik HelmChartConfig* - Bootstrap ingress controller
. *Cilium CNI HelmChartConfig* - Core networking bootstrap
. *RKE2 config.yaml.d/* - Cluster configuration
. *Systemd services* - Bootstrap infrastructure
...
*Estimated Effort*: 1-2 hours *Risk*: Low (cluster functional without
Tailscale) *Value*: High (better secret management, version control)

=== Summary Table

[width="99%",cols="22%,16%,23%,16%,10%,13%",options="header",]
|===
|Component |Location |Can Migrate? |Priority |Risk |Status
|Traefik Config |master.base |✅ Done (system/traefik) |High |CRITICAL |Applied by rke2-traefik-config.service

|OpenEBS ZFS |master.base |⚠️ Maybe |Low |HIGH |Defer (Phase 5)

|Tailscale Operator |master.base |✅ Yes |High |LOW-MED |*Recommended
(Phase 4)*

|Cilium CNI Config |master.cilium |✅ Done (system/cilium) |High |CRITICAL |Applied by rke2-cilium-config.service
...
This completes the logical migration of operational components to kpt
while keeping essential bootstrap components safely in RKE2. The
remaining components (Traefik, Core CNI, OpenEBS) are either critical
for bootstrap or too risky to migrate without specialized tooling.
