...
The Tekton Pipelines Helm chart lives in `kpt/system/tekton-pipelines` with sample setters only. `rke2-kpt-package-sync.service` mirrors this package into the downstream repo so each cluster can:

1. `kpt pkg get` the upstream package into `packagevariants/<name>/system/tekton-pipelines`.
. Update `tekton-setters.yaml` with real namespaces and non-secret chart tweaks (no credentials committed to Git).
3. Run `kpt fn render ... --output <state repo>/packagevariants/<name>/system/tekton-pipelines` and commit the rendered manifests.
. Allow `rke2-tekton-pipelines.service` to apply the rendered objects immediately after the repo sync, guaranteeing Tekton is ready before Flux starts.

Secrets stay in the Incus environment instead of Git: the systemd unit passes `TEKTON_GIT_USERNAME`, `TEKTON_GIT_PASSWORD`, `TEKTON_GIT_URL`, `TEKTON_DOCKER_CONFIG_JSON`, and `TEKTON_DOCKER_REGISTRY_URL` directly to `rke2-kpt-deploy`. Defaults derive from existing cluster variables (GitHub token, Docker config), but every value can be overridden per cluster via the cloud-init environment.

The same pattern now applies to `kpt/system/replicator` and `kpt/system/porch-resources`. Each ships sample setters in its root package so downstream clusters can clone, override with secret data (using SOPS when needed), render locally, and commit the outputs for Flux.
