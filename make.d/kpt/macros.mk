...
# Apply-setters config content for rke2 (@codebase)
...
data:
  ...
  cluster-init: "$(if $(filter master,$(node.role)),true,false)"
  cluster-name: $(cluster.name)
  cluster-token: $(cluster.token)
  node-role: $(node.role)
  node-type: $(node.type)
  node-host-ip: $(.network.node.host.inetaddr)
  pod-network-cidr: $(cluster.pod.cidr)
  service-network-cidr: $(cluster.service.cidr)
