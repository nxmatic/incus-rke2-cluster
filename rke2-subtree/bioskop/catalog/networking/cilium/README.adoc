...
This package deploys Cilium advanced networking features (BGP, L2
announcements, IP pools) for RKE2 clusters. The core CNI HelmChartConfig
remains in RKE2 cloud-config for bootstrap.
...
* *L2 Announcements*: ARP-based IP advertisement on vmnet0/lan0
interfaces
...
* *Service Advertisement*: BGP announcements for LoadBalancer IPs and
PodCIDR
...
* Requires careful testing with BGP verification and service
connectivity

=== Prerequisites

[arabic]
. *Core CNI*: RKE2 HelmChartConfig must deploy Cilium CNI first
. *BGP Gateway*: Router at 10.80.0.1 with ASN 65000 configured
. *Network Interfaces*: vmnet0 and lan0 available on control-plane nodes
. *IP Pool Ranges*: No conflicts with existing network allocations
...
cd /var/lib/incus-rke2-cluster/catalog/networking/cilium
...
* `+cluster-name+`: Cluster name for identification (default:
bioskop-cluster)
...
* `+l2-interface-1+`: Primary L2 announcement interface (default:
vmnet0)
* `+l2-interface-2+`: Secondary L2 announcement interface (default:
lan0)
...
[arabic]
. *Quick Rollback*: Delete kpt-managed resources, redeploy from
cloud-config
+
[source,bash]
----
kpt live destroy /var/lib/incus-rke2-cluster/catalog/networking/cilium
# RKE2 will recreate resources from cloud-config on next restart
----
. *Restore cloud-config*: Revert cloud-config.master.cilium.yaml changes
+
[source,bash]
----
...
----
. *Emergency Access*: If BGP fails, services remain accessible via
direct node IPs
+
[source,bash]
----
...
* *Core CNI*: HelmChartConfig in RKE2 deploys Cilium CNI (tunnel/VXLAN
mode)
* *IP Pools*: Cilium IPAM assigns LoadBalancer IPs from defined ranges
* *L2 Announcements*: ARP advertisements on vmnet0/lan0 for LoadBalancer
IPs
