...
basedir="$(dirname "$0")/../.."
netplan="${basedir}/network-plan.yaml"
if [[ ! -f "$netplan" ]]; then
  netplan="${basedir}/rke2-subtree/bioskop/network-plan.yaml"
fi
if [[ ! -f "$netplan" ]]; then
  echo "network-plan.yaml not found at $netplan; run make generate@network" >&2
  exit 1
fi
