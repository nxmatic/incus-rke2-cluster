# Generated network subnet definitions for type=node network='10.80.56.0/21' prefix=23

.network.node.eval := eval
.network.node.call := call

network.node.split.network := '10.80.56.0/21'
network.node.split.prefix := 23
network.node.split.count := 4
network.node.split.addresses := 510
network.node.split.0.network='10.80.56.0'
network.node.split.0.netmask='255.255.254.0'
network.node.split.0.prefix=23
network.node.split.0.broadcast='10.80.57.255'
network.node.split.0.addrspace='Private Use'
network.node.split.0.minaddr='10.80.56.1'
network.node.split.0.maxaddr='10.80.57.254'
network.node.split.0.addresses=510
network.node.split.0.cidr='10.80.56.0/23'
network.node.split.1.network='10.80.58.0'
network.node.split.1.netmask='255.255.254.0'
network.node.split.1.prefix=23
network.node.split.1.broadcast='10.80.59.255'
network.node.split.1.addrspace='Private Use'
network.node.split.1.minaddr='10.80.58.1'
network.node.split.1.maxaddr='10.80.59.254'
network.node.split.1.addresses=510
network.node.split.1.cidr='10.80.58.0/23'
network.node.split.2.network='10.80.60.0'
network.node.split.2.netmask='255.255.254.0'
network.node.split.2.prefix=23
network.node.split.2.broadcast='10.80.61.255'
network.node.split.2.addrspace='Private Use'
network.node.split.2.minaddr='10.80.60.1'
network.node.split.2.maxaddr='10.80.61.254'
network.node.split.2.addresses=510
network.node.split.2.cidr='10.80.60.0/23'
network.node.split.3.network='10.80.62.0'
network.node.split.3.netmask='255.255.254.0'
network.node.split.3.prefix=23
network.node.split.3.broadcast='10.80.63.255'
network.node.split.3.addrspace='Private Use'
network.node.split.3.minaddr='10.80.62.1'
network.node.split.3.maxaddr='10.80.63.254'
network.node.split.3.addresses=510
network.node.split.3.cidr='10.80.62.0/23'
