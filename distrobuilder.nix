{
  ...
  packages = {
    ...
    sets = [
      ...
      {
        packages = [
          "acl" "apt-file" "apt-transport-https" "apt-utils" "bash-completion" "bc" "binutils" "bzip2" "ca-certificates" "cloud-init" "curl" "dstat" "dnsutils" "emacs-nox" "file" "git" "gh" "gnupg" "gzip" "htop" "iproute2" "iptables" "iputils-ping" "jq" "kmod" "less" "lshw" "lsof" "man" "man-db" "nmap" "net-tools" "netcat-openbsd" "network-manager" "open-iscsi" "openssh-client" "openssh-server" "p7zip-full" "procps" "psmisc" "pv" "rsync" "socat" "strace" "sudo" "systemd" "systemd-container" "systemd-resolved" "systemd-sysv" "systemd-timesyncd" "tar" "tree" "traceroute" "tshark" "uuid-runtime" "wget" "zsh" "xz-utils"
        ];
        action = "install";
      }
    ];
